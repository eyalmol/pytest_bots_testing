...
class GetBot2Tests:
    ...
    def test_bot2_get_method_commit_default_msg():
        ...
        response = requests.get(url)
        print(response.status_code)
        assert (response.status_code == 200 and response.text == "welcome"), "something went wrong in the get method activate the bot2 function no new message"
    ...
    def test_bot2_get_method_sending_false_intent():
        ...
        response = requests.get(url)
        print(response.status_code)
        assert (response.status_code == 200 and response.text == "False intent"), "didn't return an alarm while trying to give false intent"
    ...
    def test_bot2_get_commit_fun_and_supply_new_msg():
        ...
        response = requests.get(url)
        print(response.status_code)
        assert (response.status_code == 200 and response.text == "hello_my_friend"), "was not able to generate new msg in the get bot2 functionality"
    ...
    def test_bot2_get_default_msg_supply_not_exist_id_withot_new_msg():
        ...
        response = requests.get(url)
        print(response.status_code)
        assert (response.status_code == 422), "was able to commit bot2 get default msg with wrong id and without a new msg"